...
export const addDynamicDataIntoElement = async (
  ...
  // User Settings
  await loadLoggedInUserSettingsDataIntoElements(window, user, builderDB, db, projectId, s3Url);
...
const loadLoggedInUserSettingsDataIntoElements = async (
  ...
  if (sessionAttributes) {
    if (user && user !== 'undefined' && user.userSettingId && user.userSettingId.length) {
      ...
      const loggedInUserSetting = await getUserSettingById(
        builderDB,
        db,
        projectId,
        loggedInUserSettingId,
      );
      console.log(
