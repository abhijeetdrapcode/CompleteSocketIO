...
export const deleteCustomPaymentMethod = async (req, res, next) => {
  try {
    const { db, params, stripeConfig } = req;
    const { secretKey } = stripeConfig;
    const { methodId } = params;
    const stripe = new Stripe(secretKey);
    const paymentMethod = await stripe.paymentMethods.detach(methodId);
    res.status(200).send({ paymentMethod });
    await deletePaymentMethodFromCollection(db, methodId);
  } catch (error) {
    console.log('\n error :>> ', error);
    next(error);
  }
...
const deletePaymentMethodFromCollection = async (db, methodId) => {
  try {
    let dbCollection = await db.collection(stripeCollectionName);
    await dbCollection.deleteOne({ payment_method_id: methodId });
  } catch (error) {
    console.log('\n error :>> ', error);
  }
};
