...
export const saveDocusignTokens = async (req) => {
  const { projectId, builderDB, db, body, user, decrypt } = req;
  ...
  try {
    ...
    await saveItem(builderDB, db, projectId, collectionName, tokens, user, req.headers, decrypt);
    finalData = {
      projectId,
      eventConfig,
      profile,
...
export const docusignTokenGen = async (req) => {
  try {
    const { projectId, builderDB, db, user, decrypt, environment, tenant } = req;
