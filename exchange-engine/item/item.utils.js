...
export const checkPermissionLevelSecurity = async (
  ...
  try {
    const { data: currentUser = {} } = await processAuthentication(
      builderDB,
      db,
      projectId,
      authorization,
    );
    const { permissions: userPermissions = [], userSettingId } = currentUser;
    const currentUserSetting = userSettingId && userSettingId.length ? userSettingId[0] : {};
