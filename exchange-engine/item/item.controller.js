...
export const findByQueryAndUpdateForBuilder = async (req, res, next) => {
  try {
    const { db, params, body } = req;
    const { collectionName } = params;
    const { query, parentCollectionField, itemUuid } = body;
    const result = await findOneByEqualFieldValueAndUpdate(
      db,
      collectionName,
      query,
...
export const addItemFromBuilder = async (req, res, next) => {
  try {
    const { db, params, body } = req;
    const { collectionName } = params;
    const result = await createItemFromBuilder(db, collectionName, body);
    res.status(200).send(result);
  } catch (error) {
    console.error(`error`, error);
    next(error);
...
export const findUpdateItemForBuilder = async (req, res, next) => {
  try {
    const { db, params, body } = req;
    const { collectionName, itemId } = params;
    const result = await updateItemForBuilder(db, collectionName, itemId, body);
    if (!result) {
      res.status(404).send({ message: `Record not found with id ${itemId}` });
      return;
    }
...
export const removeItemFromBuilder = async (req, res, next) => {
  try {
    const { db, params } = req;
    const { collectionName, itemId } = params;
    const result = await removeItemBuilder(db, collectionName, itemId);
    res.json(result);
  } catch (error) {
    next(error);
  }
...
export const removeReferenceItemFromBuilder = async (req, res, next) => {
  try {
    ...
    const { collectionName } = params;
    const result = await removeReferenceBuilder(db, collectionName, body);
    res.json(result);
  } catch (error) {
    next(error);
  }
...
export const addReferenceItemFromBuilder = async (req, res, next) => {
  try {
    const { db, body, params } = req;
    const { collectionName } = params;
    const result = await addReferenceBuilder(db, collectionName, body);
    res.json(result);
  } catch (error) {
    next(error);
  }
...
export const saveItemsImportedFromCSVForBuilder = async (req, res, next) => {
  try {
    const { db, body, params } = req;
    const { collectionName } = params;
    const result = await saveItemsImportedFromCSVForBuilderService(db, collectionName, body);
...
export const findOne = async (req, res, next) => {
  ...
  try {
    ...
    const result = await findItemById(db, projectId, collection, itemId);
    if (!result) {
...
export const deleteFieldRecordFromItems = async (req, res, next) => {
  try {
    ...
    await deleteFieldRecordFromItemsService(db, collectionName, fieldName);
