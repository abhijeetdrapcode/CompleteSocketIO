      ...
      req.login(user, { session: true }, async (err) => {
        ...
        try {
          ...
          if (multiTenantSaasPlugin) {
            const { tenantId, userSettingId } = user.userDetails || {};
            tenantData = tenantId && tenantId.length ? tenantId[0] : '';
            userSettingData = userSettingId && userSettingId.length ? userSettingId[0] : '';
            console.log('🚀 ~ file: user.controller.js:143 ~ req.login ~ tenantData:', tenantData);
            console.log('user.controller.js:157 ~ req.login ~ userSettingData:', userSettingData);
          }
          /**
...
export const generateSecretCode = async (req, res) => {
  ...
  if (!twoAuthPlugin) {
    return res.status(4200).json(req.user);
  }
